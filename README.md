...
### 方法 A: zip 一括版を使う（Python 不要・おすすめ）

1. [Releases](https://github.com/kozimusafu-create/markdown-viewer/releases) から `markdown-viewer-v1.0.0-win.zip` をダウンロード
2. 好きな場所に展開（exe・セットアップスクリプト・手順書が同梱）
...
> exe 単体だけ欲しい場合は Release の `md_viewer.exe` を直接ダウンロードしてもよい（その場合は `setup_md_viewer.ps1` を同じフォルダに置くこと）。
...
PyInstallerでビルドしたexeはセキュリティソフトに誤検知されることがあります。
ソースコード（md_viewer.py）を確認の上、自分でビルドして使うことも可能です。
